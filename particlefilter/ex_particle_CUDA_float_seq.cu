...
__global__ void normalize_weights_kernel(double *weights, int Nparticles,
  ...
  if (0 == threadIdx.x) sumWeights = partial_sums[0];
  ...
  if (i == 0) {
    ...
    u[0] = (1 / ((double)(Nparticles))) * d_randu(seed, i);  // do this to allow
                                                             // all threads in
                                                             // all blocks to
                                                             // use the same u1
  }
...
__global__ void likelihood_kernel(double *arrayX, double *arrayY, double *xj,
  ...
  if (i < Nparticles) {
    ...
    weights[i] = 1 / ((double)(Nparticles));  // Donnie - moved this line from
                                              // end of find_index_kernel to
                                              // prevent all weights from being
  ...
  if (i < Nparticles) {
    for (y = 0; y < countOnes; y++) {
      ...
      if (ind[i * countOnes + y] >= max_size) ind[i * countOnes + y] = 0;
    }
    likelihood[i] = calcLikelihoodSum(I, ind, countOnes, i);

    likelihood[i] = likelihood[i] / countOnes;

    weights[i] = weights[i] * exp(likelihood[i]);  // Donnie Newell - added the
                                                   // missing exponential
                                                   // function call
  }
